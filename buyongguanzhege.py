...
def superm2(image):
    ...
    for iter in range(10):
        ...
        for match, match2 in matches:
            ...
            houghth[i] = theta
            weights[i] = Mij
            total_houghr.append(houghr)
            total_houghth.append(houghth)
            total_weights.append(weights)
            i += 1

    total_houghr = np.array(total_houghr)
    total_houghth = np.array(total_houghth)
    totall_weights = np.array(total_weights)
    total_houghr = np.mean(total_houghr, axis=0)
    total_houghth = np.mean(total_houghth, axis = 0)
    total_weights = np.mean(totall_weights, axis = 0)

    for i in range(len(matches)):
        houghr[i] = total_houghr[i]
        houghth[i] = total_houghth[i]
        weights[i] = total_weights[i]
